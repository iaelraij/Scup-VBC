...
function[]=domain_w(fname,prevRunN,runN,nodes,targetH)
targetH={num2str(targetH)};
% fid=fopen([fname prevRunN '\conc1.out']);
% conc=fread(fid,inf,'float');
% conc_final=conc(nodes+3:(nodes*2+2),:);

fid=fopen([fname prevRunN '\h.out']);
...
fid = fopen([fname prevRunN '\DOMAIN.dat'], 'r');%CHECK that this file is updated!!!
i = 1;
lines{i} = fgets(fid);
